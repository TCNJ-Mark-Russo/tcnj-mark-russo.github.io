 ...
 * title   : Title of the question (text)
 * prompt  : An array of HTML lines containing labels to be substituted with dropdowns (label like __Q01A)
 * options : a map from label to array of options to choose from for each question
 * solution: a map from label to correct solution from each associated options array
 */
...
class QFillInBlanks {
    constructor(q) {
        ...
        <div class="prompt" style="padding:5px;"></div>
        ...
        let topDiv     = this.qdoc.querySelector(`#${this.qid}`);
...
/**
 * QMultipleChoice
 * q.id is a unique identifier for this question
 * q.title is the question title
 * q.prompt is and HTML string that prompts to answer the question
 * q.solution is an array of two-element arrays, where each two-element array is 
 *   (1) an answer and (2) a boolean indicating if the answer should be selected.
 */
